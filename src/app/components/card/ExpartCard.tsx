  ...
  return (
    <div className='w-[420px] rounded shadow-custom-light border-b-2 border-b-slate-200'> {/* Adjusted to shadow-lg */}
      <div className="relative w-[420px] h-full image-shape">
        ...
        />
        <div
          className={`absolute bottom-0 left-0 w-full h-[50%] transition-all duration-300 ${
      ...
      <div className="flex flex-col justify-between relative p-6">
          ...
          <div className="pt-10 pb-12 px-8">
            <p
              className={`text-xl font-bold font-poppins transition-colors duration-300 ${
                hoveredIndex === index ? 'text-[#E67529]' : 'text-black'
              }`}
            >
              {title}
            </p>
        ...
        <div className="relative flex items-center justify-center rounded-lg">
          {shareIconHoveredIndex !== index ? (
            ...
            <div
              className="absolute left-[50px] bottom-44 flex flex-col rounded-3xl items-end space-y-7 bg-[#333333] py-5 px-7 text-white"
              onMouseEnter={() => setShareIconHoveredIndex(index)}
              onMouseLeave={() => setShareIconHoveredIndex(null)}
            >
